...
def to_tool_id(name):
    ...
    return name_to_id.get(key, None)
    ...
    for vi, vid in enumerate(videos, 1):
        ...
        for idx_s, objs in labels.items():
            ...
            if idx - (SEQ_LEN - 1) < 0 or idx >= fcount:
                print(f"    [SKIP] Frame {idx}: SEQ_LEN boundary check")
                continue

            gt_pairs = build_gt_pairs_dict(objs)  # {(tool_id, tti_id): is_tti}
            if len(gt_pairs) == 0:
                print(f"    [SKIP] Frame {idx}: gt_pairs empty")
                continue
            
            print(f"    [PROCESS] Frame {idx}: gt_pairs={gt_pairs}")
            ...
            for d in dets:
                ...
                tid = to_tool_id(nm)
                xid = to_tti_id(nm)
                # print(f"        Processing cls={cls} nm='{nm}' -> tid={tid}, xid={xid}")
                if tid is not None:
                    d['tool_id'] = tid
                    tools.append(d)
                    # print(f"          Added to tools (tid={tid})")
                elif xid is not None:
                    d['tti_id'] = xid
                    tissues.append(d)
                    # print(f"          Added to tissues (xid={xid})")

            pairs = pair_by_k_nearest(tools, tissues, k=K_NEAREST, d_max=D_MAX)
            total_pairs += len(pairs)
            
            print(f"      Tools: {len(tools)}, Tissues: {len(tissues)}, Pairs: {len(pairs)}")
            ...
            for pair in pairs:
                ...
                print(f"      Pred pair: {key}, in gt_pairs: {key in gt_pairs}")
