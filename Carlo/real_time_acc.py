...
VIDEO_PATH   = "../Dataset/video_dataset/videos/test/Utswset-Case 6-002.mp4"
...
ALPHA_TOOL   = 0.50
ALPHA_TTI    = 0.50
COLOR_TOOL   = (255, 140, 60)   # BGR
COLOR_TTI    = (60, 220, 60)    # BGR
COLOR_BOX_OK = (40, 200, 40)    # BGR
FONT         = cv2.FONT_HERSHEY_SIMPLEX

def to_tool_id(name):
    if name is None: return 0
    d = {'unknown_tool':0,'dissector':1,'scissors':2,'suction':3,
         'grasper 3':4,'harmonic':5,'grasper':6,'bipolar':7,'grasper 2':8,
         'cautery (hook, spatula)':9,'ligasure':10,'stapler':11}
    return d.get(str(name).strip().lower(), 0)

def to_tti_id(name):
    if name is None: return 12
    d = {'unknown_tti':12,'coagulation':13,'other':14,'retract and grab':15,
...
def main():
    ...
    out = cv2.VideoWriter(OUTPUT_PATH, fourcc, FPS/WRITE_EVERY if WRITE_EVERY>1 else FPS, (W, H))
    ...
    while True:
        ...
        need_process = (idx % PROCESS_EVERY == 0)
        if (not need_process) and (last_viz is not None):
            if WRITE_EVERY == 1 or (write_ctr % WRITE_EVERY == 0):
                out.write(last_viz)
            write_ctr += 1
            continue

        r = yolo.predict(source=fr_bgr, imgsz=YOLO_IMGSZ, conf=CONF_THR,
                         half=(DEVICE.type=='cuda'), verbose=False)
        ...
        for d in dets:
            ...
            if to_tool_id(nm) != 0:
                d['name'] = nm; tools.append(d)
            elif to_tti_id(nm) != 12:
                d['name'] = nm; tissues.append(d)
        ...
        if (idx // PROCESS_EVERY) % max(1, DEPTH_EVERY) == 0 or last_depth is None:
            if 0 < DEPTH_DOWNSCALE < 1.0:
                small = cv2.resize(fr_bgr, (0,0), fx=DEPTH_DOWNSCALE, fy=DEPTH_DOWNSCALE, interpolation=cv2.INTER_AREA)
                d_small = depth_pipe(Image.fromarray(cv2.cvtColor(small, cv2.COLOR_BGR2RGB)))['depth']
                d_small = np.array(d_small, dtype=np.float32)
                last_depth = cv2.resize(d_small, (W, H), interpolation=cv2.INTER_LINEAR)
            else:
                d_full = depth_pipe(Image.fromarray(cv2.cvtColor(fr_bgr, cv2.COLOR_BGR2RGB)))['depth']
                last_depth = np.array(d_full, dtype=np.float32)
        ...
        if len(frame_buffer) == SEQ_LEN and tools and tissues:
            ...
            for pr in pairs:
                ...
                clips.append(seq); boxes.append(box)

            if clips:
                xb = torch.from_numpy(np.stack(clips, axis=0)).to(DEVICE)  # [P,T,5,224,224]
                with torch.inference_mode(), torch.cuda.amp.autocast(enabled=(DEVICE.type=='cuda')):
                    probs = torch.sigmoid(tcn(xb).squeeze(1)).detach().cpu().numpy()
                for box, p in zip(boxes, probs):
                    if p >= THR_TCN:
                        draw_box_with_text(viz, box, f"TTI=1  p={p:.2f}", COLOR_BOX_OK)
        ...
        if (idx % 150) == 0 and DEVICE.type == 'cuda':
