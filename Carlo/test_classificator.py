...
def build_gt_pairs_and_masks(objs, H, W):
    ...
    for o in objs:
        if 'instrument_polygon' in o:
            ...
            tmask = poly_to_mask(get_polygon(o['instrument_polygon']), H, W)
            tools.append((tid, tmask))
        if 'tti_polygon' in o:
            uid = to_tti_id(o.get('tti_class'))
            is_tti = int(o.get('is_tti', 0))
            inter_tid = to_tool_id(o.get('interaction_tool'))
            umask = poly_to_mask(get_polygon(o['tti_polygon']), H, W)
            tissues.append((uid, is_tti, inter_tid, umask))
    pairs = []
    ...
    return pairs  # lista di coppie GT + maschere

def evaluate_oracle(depth_pipe, model, labels_dir, videos_dir):
    model.eval()
    y_true, y_prob = [], []
    ...
    for vid in videos:
        ...
        print(f"[ORACLE] Processing video: {vid}  ({fcount} frames)")

        for idx_s, objs in labels.items():
            idx = int(idx_s)

            print(f"   Frame {idx}/{fcount}")
            ...
            depth_map = np.array(depth_pipe(Image.fromarray(cv2.cvtColor(frame_c_bgr, cv2.COLOR_BGR2RGB)))['depth'],
                                 dtype=np.float32)

            for (_, _, label, tmask, umask) in pairs:
                bbox = extract_union_bbox(tmask, umask)
                if bbox is None:
                    continue
                seq = make_clip(cap, idx, bbox, depth_map, tmask, umask)
                xb = torch.from_numpy(seq).unsqueeze(0).to(DEVICE)  # [1,T,C,H,W]
                with torch.no_grad():
                    logit = model(xb).squeeze(1)  # [1]
    ...
    if not y_true:
        print("ORACLE: nessun sample raccolto.")
        return 0.5, {}
